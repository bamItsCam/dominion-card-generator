...
//window.addEventListener('load', function() {
    ...
    function onChangeExternalImage(id, value, maxWidth, maxHeight) {
        ...
        if(url.length>0 && useCORS){
            loadImgAsBase64(url, (dataURL) => { setImageSource(dataURL) }, maxWidth, maxHeight); 
        } else { 
            setImageSource(url);
        }
    }
...
function loadCardList() {
    ...
    allStoredCards = storage.getAll(function(error, data) {
        if (error) throw error;
        // place the cardKey into each card object as a field, then cast to an array
        ...
        savedCardsArray.forEach((card) => {
            insertSingleCardIntoList(card)
        })
    })
}

function insertSingleCardIntoList(card) {
    table = document.getElementById("cardList")
    ...
    nameC.innerHTML = card.name
    delC.innerHTML = '<td><button type="button" onclick="deleteCard(this)">X</button></td>'
}
...
function addCard() {
    ...
    if (form.elements["name"].value == "") {
        alert("'Card Name' is a required field")
        return false
    }
    ...
    cardId = ("cardId_" + epoch.toString()).toString()
    console.log(cardId)
    
    storage.set(cardId, { name: form.elements["name"].value }, function(error) {
        console.log(form.elements['name'].value)
        if (error) {
            throw error
        }
        else {
            card = Object.values(form.elements).reduce((obj,field) => {obj[field.name] = field.value; return obj},{})
            card.id = cardId
            console.log(card)
            insertSingleCardIntoList(card)
        }
...
function saveCardFromParams(card) {
    
}

function deleteCard(td) {
    storage.remove(td.parentNode.parentNode.id, function(error) {
        if (error) {
            console.log(td.parentNode.parentNode.id)
            throw error
        }
        else {
...
diskController.saveCard = function(card) {
    var epoch = new Date().getTime()
    console.log(card)
    cardId = "cardId_" + epoch.toString()
    storage.set(cardId, card, function(error) {
        if (error) {
            throw error
        }
        else {
...
domController.updateCardPreview = function() {
