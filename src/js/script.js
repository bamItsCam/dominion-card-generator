...
const os = require('os');
const storageService = require('electron-json-storage');
var entrypoints = {}
var cardController = {}
var domService = {}
var helpers = {}
...
cardController.clearList = function(){
    storageService.clear(function(error) {
        if (error) {
            throw error;
        }
        else {
...
cardController.deleteCard = function(cardId) {
    storageService.remove(cardId, function(error) {
        if (error) {
            throw error
        }
        else {
    ...
    for (var cardKey in card) {
        switch (cardKey) {
            case "color0":
                ...
                break;
            case "color1":
                normalColorCurrentIndices[1] = normalColorDropdowns[1].selectedIndex = card[cardKey];
                break;
            case "size":
                var buttonElement = document.getElementsByName("size")[templateSize = parseInt(card[cardKey])];
                document.body.className = buttonElement.id;
                buttonElement.checked = true;
                break;
            default:
                var matches = cardKey.match(/^c(\d)\.(\d)$/);
                if (matches) {
                    var id = matches[1];
                    ...
                    recolorFactorList[id][matches[2]] = recolorInputs[12*id + parseInt(matches[2])].value = parseFloat(card[cardKey]);
                } else {
                    matches = cardKey.match(/^c(\d)\.(\d)\.(\d)$/);
                    if (matches) {
                        ...
                        recolorFactorList[id][parseInt(matches[2]) * 3 + parseInt(matches[3])] = recolorInputs[12*id + 3*parseInt(matches[2]) + parseInt(matches[3])].value = parseFloat(card[cardKey]);
                    } else {
                        var el = document.getElementById(cardKey);
                        if (el)
                            el.value = card[cardKey];
                    }
                }
                break;
        }
